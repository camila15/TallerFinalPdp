...
const Game = () => {
  ...
  useEffect(() => {
    fetch(
      Constant.BASE_URL+`amount=10&category=${category}&difficulty=${difficult}&type=multiple`
    )
      .then((res) => res.json())
      .then((data) => {               //get
    ...
    <div>
      ...
      <div className="card_game">
        ...
        <br></br><br/>
          ...
          <tr>
            <td>
            <button class="btn btn-primary btn-lg btn-block optionsbtn" onClick={cambios}>
              {preguntas[indice] ? preguntas[indice].correct_answer : ""}
            </button>
            </td>
            <br></br>
            <td>
            <button class="btn btn-primary btn-lg btn-block optionsbtn" onClick={fail}>
